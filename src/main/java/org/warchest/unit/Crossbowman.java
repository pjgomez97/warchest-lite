...
public class Crossbowman implements Unit, StandardUnit {
    ...
    public boolean canAttack(Square origin, Square target) {
        return  (Math.abs(origin.getPosition().row() - target.getPosition().row()) <= 2 && Math.abs(origin.getPosition().column() - target.getPosition().column()) == 0) ||
                (Math.abs(origin.getPosition().row() - target.getPosition().row()) == 0 && Math.abs(origin.getPosition().column() - target.getPosition().column()) <= 2);
    }
